...
import com.buzbuz.smartautoclicker.core.domain.model.condition.DEFAULT_REQUIRED_DETECTION_COUNT
...
class ImageConditionViewModel @Inject constructor(
    ...
    /** The detection interval currently edited by the user. */
    val detectionIntervalMs: Flow<Long> = configuredCondition.map {
        it.detectionIntervalMs.coerceIn(MIN_DETECTION_INTERVAL_MS.toLong(), MAX_DETECTION_INTERVAL_MS.toLong())
    }

    /** The required detection count currently edited by the user. */
    val requiredDetectionCount: Flow<Int> = configuredCondition.map {
        it.requiredDetectionCount.coerceIn(
            DEFAULT_REQUIRED_DETECTION_COUNT,
            MAX_REQUIRED_DETECTION_COUNT.toInt(),
        )
    }

    /** Tells if the detection counter resets after the condition is fulfilled. */
    val resetAfterTrigger: Flow<Boolean> = configuredCondition.map { it.resetAfterTrigger }
    ...
    /**
     * Set the detection interval of the configured condition.
     * @param value the new detection interval in milliseconds.
     */
    fun setDetectionInterval(value: Long) {
        updateEditedCondition { oldCondition ->
            oldCondition.copy(
                detectionIntervalMs = value.coerceIn(
                    MIN_DETECTION_INTERVAL_MS.toLong(),
                    MAX_DETECTION_INTERVAL_MS.toLong(),
                )
            )
        }
    }
    ...
    /**
     * Set the required detection count of the configured condition.
     * @param value the new required detection count.
     */
    fun setRequiredDetectionCount(value: Int) {
        updateEditedCondition { oldCondition ->
            oldCondition.copy(
                requiredDetectionCount = value.coerceIn(
                    DEFAULT_REQUIRED_DETECTION_COUNT,
                    MAX_REQUIRED_DETECTION_COUNT.toInt(),
                )
            )
    ...
    /** Set if the detection counter should reset after the condition is fulfilled. */
    fun setResetAfterTrigger(value: Boolean) {
        updateEditedCondition { oldCondition ->
            oldCondition.copy(resetAfterTrigger = value)
        }
    }

    /** Toggle if the detection counter should reset after the condition is fulfilled. */
...
const val MIN_DETECTION_INTERVAL_MS = 100f
const val MAX_DETECTION_INTERVAL_MS = 5000f
const val MAX_REQUIRED_DETECTION_COUNT = 50f
