...
internal class ConditionsVerifier(
    ...
    /** Tracks interval-based successful detection counters per ImageCondition. */
    private val detectionCounterStates: MutableMap<Long, DetectionCounterState> = mutableMapOf()
    ...
    private suspend fun verifyImageCondition(condition: ImageCondition): ProcessedConditionResult.Image {
        ...
        val currentTsMs = currentVerificationTsMs ?: System.currentTimeMillis()
        val conditionKey = condition.getValidId()
        val counterState = detectionCounterStates.getOrPut(conditionKey) { DetectionCounterState() }
        val detectionIntervalMs = condition.detectionIntervalMs.coerceAtLeast(1L)

        if (currentTsMs - counterState.lastCheckTimestamp < detectionIntervalMs) {
            val pendingResult = condition.toInvalidConditionResult()
            progressListener?.onImageConditionProcessingCompleted(pendingResult)
            return pendingResult
        }
        counterState.lastCheckTimestamp = currentTsMs
        ...
        val rawResult: ProcessedConditionResult.Image = bitmap?.let { conditionBitmap ->
        ...
        if (rawResult.isFulfilled) {
            counterState.count++
        }

        val requiredDetectionCount = condition.requiredDetectionCount.coerceAtLeast(1)
        val result = if (counterState.count >= requiredDetectionCount) {
            if (condition.resetAfterTrigger) counterState.count = 0
            rawResult.copy(isFulfilled = true)
        } else {
            rawResult.copy(isFulfilled = false)
        }
...
internal data class DetectionCounterState(
    var count: Int = 0,
